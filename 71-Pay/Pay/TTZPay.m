...
- (void)paymentQueueRestoreCompletedTransactionsFinished:(SKPaymentQueue *)queue {
    ...
    if(!queue.transactions.count) {
        ...
        [self.products enumerateKeysAndObjectsUsingBlock:^(NSString * key, NSString * obj, BOOL * _Nonnull stop) {
            [TTZKeyChain removeDataForkey:key];
        }];
        return;
    }

    NSMutableDictionary *tempProduct = self.products.mutableCopy;
    
    for (SKPaymentTransaction *transaction in queue.transactions)
    {
        NSString *productID = transaction.payment.productIdentifier;
        [tempProduct removeObjectForKey:productID];
        NSLog(@"-------------5.0 恢复购买产品ID:%@",productID);
    }
    
    [tempProduct enumerateKeysAndObjectsUsingBlock:^(NSString * key, NSString * obj, BOOL * _Nonnull stop) {
        [TTZKeyChain removeDataForkey:key];
    }];
                                         ...
                                         dispatch_async(dispatch_get_main_queue(), ^{
                                             ...
                                             if (error) {
                                                 ...
                                                 if(status){
                                                     ...
                                                     if(status == 21007){
                                                         NSLog(@"-------------6.0 验证出错-收据信息是测试用（sandbox），但却被发送到产品环境中验证:%ld",(long)status);
                                                         [self verifyReceiptAgainData:requestData];
                                                         return;
                                                     }
...
//FIXME:  -  本地二次验证订单(沙盒)
- (void)verifyReceiptAgainData:(NSData *)requestData{
    
    NSLog(@"-------------6.0 二次验证中...");
    !(_loadingBuy)? : _loadingBuy(@"二次验证中...");
    ...
    [[[NSURLSession sharedSession] dataTaskWithRequest:storeRequest
                                     completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
                                         ...
                                         dispatch_async(dispatch_get_main_queue(), ^{
                                             ...
                                             if (error) {
                                                 NSLog(@"-------------6.0 二次请求失败:%@",error.localizedDescription);
                                                 !(_payComplete)? : _payComplete([NSString stringWithFormat:@"请求失败:%@",error.localizedDescription]);
                                                 ...
                                                 NSDictionary *jsonResponse = [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
                                                 if (!jsonResponse) {
                                                     NSLog(@"-------------6.0 二次验证失败:%@",error.localizedDescription);
                                                     !(_payComplete)? : _payComplete([NSString stringWithFormat:@"验证失败:%@",error.localizedDescription]);
                                                 ...
                                                 NSInteger status = [[jsonResponse valueForKey:@"status"] integerValue];
                                                 if(status){
                                                     NSLog(@"-------------6.0 二次验证出错:%ld",(long)status);
                                                     !(_payComplete)? : _payComplete([NSString stringWithFormat:@"\n验证出错:%ld\n",(long)status]);
                                                     return;
                                                 }
                                                 //NSString *environment = [jsonResponse valueForKey:@"environment"];
