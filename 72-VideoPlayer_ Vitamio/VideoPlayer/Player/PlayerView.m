...
#import <VitamioSDK-iOS/VMediaPlayer.h>
...
- (void)horizontalMoved:(CGFloat)value {
    ...
    // 需要限定sumTime的范围
    NSTimeInterval totalMovieDuration           = self.mediaPlayer.getDuration;
...
{
    NSLog(@"%s--获取到视频的相关信息--时长：%ld秒", __func__,player.getDuration/1000);

    [player start];
    
    long total = self.mediaPlayer.getDuration/1000;
    BOOL islive = !(total > 0);
...
{
    NSLog(@"NAL 1RRE &&播放器播放失败&& VMediaPlayer Error: %@", arg);
    NSString *errorMsg = [self error:[NSString stringWithFormat:@"%@",arg]];
...
//    self.progressDragging = NO;
    ...
    if(self.mediaPlayer.getDuration/1000) [self timer];
    NSLog(@"NAL 3HBT &&&&&&&&&&&&&&&&...缓冲完成....&&&&&&&&&&&&&&&&&");
}
...
- (void)videoDurationChange:(SPVideoSlider *)sender{
    NSLog(@"%s", __func__);
    [self.mediaPlayer seekTo:sender.value * self.mediaPlayer.getDuration];
}
