...
+ (void)startNetworkMonitoring{
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        if (!isOnLine) {[[LZHTTP sharedInstance] appStatus];}
        
        [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"WWANKey"];
        
        [LZHTTP sharedInstance].NotReachable = NO;
        ...
        switch (status) {
            ...
            case AFNetworkReachabilityStatusNotReachable:
                ...
                [LZHTTP sharedInstance].NotReachable = YES;
                break;
            
            case AFNetworkReachabilityStatusReachableViaWWAN:
            
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"WWANKey"];
                break;
...
- (void)getRequest:(NSString *)urlString
    ...
    if(LZHTTP.isProtocolService) return;
          ...
          parameters:nil progress:nil
             success:nil failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
                 if (([error.localizedDescription containsString:@"404"] || [error.localizedDescription containsString:@"not found"])) {
                     ...
                     if ([LZHTTP sharedInstance].NotReachable) return;
