...
@interface SPIQYVideo()<WKNavigationDelegate>
@property (nonatomic, strong)  WKWebView*webView;
@property (nonatomic, copy) void(^completeBlock)(NSString *url);
...
- (void)playWithURL:(NSString *)urlString
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSMutableURLRequest *mutableReqeust = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlString]];
        ...
        [self.webView loadRequest:mutableReqeust];
        
        [self performSelector:@selector(reset) withObject:nil afterDelay:20.0];
    });
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    ...
    [webView evaluateJavaScript:JsStr completionHandler:^(id _Nullable response, NSError * _Nullable error) {
        if(![response isEqual:[NSNull null]] && response != nil){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([SPIQYVideo sharedVideo].completeBlock) {
                    [SPIQYVideo sharedVideo].completeBlock(response);
                    [SPIQYVideo sharedVideo].completeBlock = nil;
                }
            });
        }else{
            //没有视频链接
...
//                  ||[requestUrl containsString:@".mp4"]
           )
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([SPIQYVideo sharedVideo].completeBlock) {
                    [SPIQYVideo sharedVideo].completeBlock([urlArray lastObject]);
                    [SPIQYVideo sharedVideo].completeBlock = nil;
                }
