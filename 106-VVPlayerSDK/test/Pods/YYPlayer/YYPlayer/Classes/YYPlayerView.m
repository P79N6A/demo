...
typedef NS_ENUM(NSInteger, VVPlayerPanDirection){
    VVPlayerPanDirectionHorizontalMoved, // 横向移动
    VVPlayerPanDirectionVerticalMoved    // 纵向移动
};

typedef NS_ENUM(NSUInteger, VVPlayerState) {
    VVPlayerStateSmall,
    VVPlayerStateAnimating,
    VVPlayerStateFullScreenRight,
    VVPlayerStateFullScreenLeft,
};
...
/** 屏幕状态 */
@property (nonatomic, assign) VVPlayerState state;
...
/** 平移方向 */
@property (nonatomic, assign) VVPlayerPanDirection           panDirection;
/** 用来保存快进的总时长 */
...
+ (instancetype)playerView{
    NSBundle *viewBundle = [NSBundle bundleForClass:self];
    return [viewBundle loadNibNamed:NSStringFromClass(self) owner:nil options:nil].firstObject;
}
...
- (void)changeRotate:(NSNotification*)noti {
    ...
    if(self.state == VVPlayerStateAnimating) return;
    ...
    if (self.state == VVPlayerStateSmall) {
    ...
    }else  if (self.state != VVPlayerStateSmall){
        switch (orientation) {
...
- (void)panDirection:(UIPanGestureRecognizer *)pan {
    ...
    if(self.state == VVPlayerStateSmall) return;
    if(self.lockBtn.isSelected) return;
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateBegan:{ // 开始移动
            ...
            if (x > y) { // 水平移动
                // 取消隐藏
                self.panDirection = VVPlayerPanDirectionHorizontalMoved;
                // 给sumTime初值 (点播)
                if(!self.videoButtomView.isHidden) self.sumTime = self.mediaPlayer.currentPosition;
            }
            else if (x < y){ // 垂直移动
                self.panDirection = VVPlayerPanDirectionVerticalMoved;
                // 开始滑动的时候,状态改为正在控制音量
...
- (void)enterFullscreenLeft {
    ...
    if (self.state == VVPlayerStateFullScreenLeft) {
        return;
    }
    ...
    self.state = VVPlayerStateAnimating;
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        self.state = VVPlayerStateFullScreenLeft;
    }];
...
- (void)enterFullscreenRight {
    ...
    if (self.state == VVPlayerStateFullScreenRight) {
        return;
    }
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        self.state = VVPlayerStateFullScreenRight;
    }];
